...
function setup() {
  ...
  // we're going to use the "web assembly" version
  // of tensorflow.js, which should be a lot faster
  // but we need to make sure everything is loaded first
  while(!tf.ready()) {
    // this while-loop will just repeat until
    // everything is ready for us
  }
  
  // if you still get an error, you might try changing which
  // version of tensorflow is being used in the index.html
  // file – there's a note there on what to change!
